...
namespace AgroVA.Infra.IoC
{
    ...
    {
        public static IServiceCollection AddInfrastructureAPI(this IServiceCollection services, IConfiguration configuration, IWebHostEnvironment env)
        {
            if (env.IsStaging())
            {
                services.AddDbContext<ApplicationDbContext>(options =>
                    options.UseInMemoryDatabase("AGROVA"));
            }
            else
            {
                services.AddDbContext<ApplicationDbContext>(options =>
